...
RUN echo "deb http://packages.elastic.co/elasticsearch/1.0/debian stable main" > /etc/apt/sources.list.d/elasticsearch.list
RUN apt-get update

RUN apt-get install -y graphite-carbon
RUN echo "CARBON_CACHE_ENABLED=true" > /etc/default/graphite-carbon

RUN apt-get -y install graphite-web apache2 apache2-mpm-worker libapache2-mod-wsgi
...
RUN update-rc.d elasticsearch default

RUN apt-get install -y nodejs npm
RUN ln -s /usr/bin/nodejs /usr/local/bin/nodejs
RUN curl https://codeload.github.com/grafana/tar.gz/v1.7.0 | tar -xz
